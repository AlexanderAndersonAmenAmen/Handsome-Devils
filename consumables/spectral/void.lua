local function hnds_void_target_limit(card)
...
local function void_selected_cards(card, forced_cards)
...
local function is_negative(target)
    return target and target.edition and (target.edition.negative or target.edition.key == "e_negative")
end

local function apply_void(card, cards)
    local targets = {}
    ...
    G.GAME.hnds_void_hand_penalty = (G.GAME.hnds_void_hand_penalty or 0) + 1
...
SMODS.Consumable({
    ...
    loc_vars = function(self, info_queue, card)
        ...
        local target_limit = hnds_void_target_limit(card)
        local bonus = HNDS and HNDS.get_contagion_bonus and HNDS.get_contagion_bonus() or 0
        return {
            key = bonus > 0 and "c_hnds_void_contagion" or nil,
            vars = { target_limit, 1 },
        }
    end,
    discovered = false,
    ...
    use = function(self, card, context, copier)
        apply_void(card, void_selected_cards(card))
    end,
    can_use = function(self, card)
        ...
        if not (G.hand and #G.hand.highlighted > 0 and #G.hand.highlighted <= hnds_void_target_limit(card)) then
            return false
        end
    ...
    force_use = function(self, card, area)
        local cards = Cryptid and Cryptid.get_highlighted_cards and Cryptid.get_highlighted_cards({ G.hand }, {}, 1, hnds_void_target_limit(card)) or void_selected_cards(card)
        apply_void(card, cards)
    end,
